...
description: >-
  Escribe el test primero, míralo fallar por la razón correcta, implementa lo
  mínimo. Úsala al ejecutar cualquier tarea de implementación o corrección, al
  derivar un plan de tareas, y siempre que tengas que decidir si un verde está
  de verdad verificado. Incluye la regla del andamio (un fallo de importación no
  es un RED), la anatomía de una tarea que no explota y el cálculo del radio de
  un cambio.
metadata:
  origin: one-markdown
